...
$("#start-audioTX").click(function () {
	if (!audioTXnode.process_out) Init_Audio_TX();
...
function Lance_websocket_audioTX() {
    ...
    var adresse = "ws://" + Host_IP.TX + ":" + (parseInt(SDR_RX.port_audio) + 4).toString() + "/"; // Base port number +4 for TX audio
...
async function Connect_TX_Nodes() {
    await audioTX.Ctx.audioWorklet.addModule('JS/remote_TX_audio_Processor.js?tt=5'); //Separate file containig the code of the AudioWorkletProcessor
    CW_play = [];
    CW_KeyLeft = {
        Ton: 0,
        Toff: 0,
            ...
            if (websocket.audioTXpret) {
                if (audioTX.Transmit) {
                    if (!audioTX.Before)
                        ...
                        websocket.audioTX.send(OutData); //Stopping the sending of data cuts the power of the transmitter
                }
...
function choice_TX_Source() {
    ...
    if (audioTXnode.process_out != null) { //Last node created
		...
		if (audioTX.Mic_or_Aux ==0) { //Microphone as input
			...
			if (audioTX.AutoCorrection)  audioTX.Source=1; //Sinus transmitted  to align TX freq on RX
...
function GainTXaudio(){
	...
	audioTXnode.Mic_gain.gain.value = 0.2 * audioParaTX.VolMic / Math.max(1, audioTX.Source); //Divide by 2 if 2 tone test
	audioTXnode.Aux_gain.gain.value = audioParaTX.VolAux;
}
function TX_SourceId(v){ //Create Auxiliry Source
	v = Math.min(audioTX.devicein.length-1,v); //If number of media devices has changed
	audioParaTX.In_aux=v;
	if(v<audioTX.devicein.length && audioTX.Ctx!=null){
		let id= audioTX.devicein[v][1];
		var audioConstraint={deviceId: { exact: id }}
...
function dtmf_d(x) {
    if (audioTX.mic_stream != null) {
        ...
        if (x == 1750) { //Not dtmf but single tone 1750Hz
            audioTXnode.Mic_gain.gain.value = 0.2 * audioParaTX.VolMic; //-14dB to avoid saturation at intput
            audioTX.osc2.disconnect();
            var Fl = x;
        } else {
            audioTX.osc2.connect(audioTXnode.Mic_gain);
            audioTXnode.Mic_gain.gain.value = 0.2 * audioParaTX.VolMic / 2; //Divide by 2 as 2 tones
            var Fc = 1209;
            if (x == "2" || x == "5" || x == "8" || x == "0")
                Fc = 1336; //Hz
            if (x == "3" || x == "6" || x == "9" || x == "#")
