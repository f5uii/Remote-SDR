...
function drawFFT() {
    dessin = requestAnimationFrame(drawFFT);
    ...
    if (audioRX.cptFFT == 0) {
        ...
        F_Audio_Top = Plot_Table("equalizerFFTRX", array_freq_domain, audioRX.Ctx.sampleRate, 5000, "white", 1, false, true);
    }
};
...
function Connect_RX_Nodes() {
    ...
    audioRXnode.gain_main.connect(audioRXnode.analFFT);
...
$("#start-audio").click(function () {
    ...
    if (!audioTXnode.process_out)
        Init_Audio_TX(); //Prepare also the TX. Needed in some case for external audio
})
// The Websockets
//***************
function Lance_Websocket_audio() {
    ...
    var adresse = "ws://" + Host_IP.RX + ":" + parseInt(SDR_RX.port_audio).toString() + "/"; //Port serveur web de base  pour l'audio
    websocket_audio = new WebSocket(adresse);
    $("#RX_audi_set").css("background-color", "LightGreen");
    console.log(Date.now() - T0_remsdr, "RX audio socket setting");
    ...
    websocket_audio.onopen = function (evt) {
        ...
        console.log(Date.now() - T0_remsdr, "RX audio socket open");
    };
    // code when server send a message
    websocket_audio.onmessage = function (evt) {
        ...
        reader.addEventListener("loadend", function () {
            ...
            if (!audioRX.Started) {
                ...
                for (var i = 0; i < 10; i++) {
                    audioRX.Tampon0[i] = 0; //Reset Buffer at start
                }
            }
            // Volume RX Audio
            ...
            for (var i = 0; i < audio.length; i++) {
                var v = audio[i] / 32768; ; //Audio are signed 16 bits  samples
            ...
            if (audioRXnode.source != null) {
				audioRXnode.source.port.postMessage({
                    BufferMax: audioRX.BufferMax.toString()
                });
                audioRXnode.source.port.postMessage({
