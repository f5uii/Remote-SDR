        ...
        write(chunk) {
            ...
            while (lines.length > 1) {
                ...
                if (message.indexOf("D") >= 0 || message.indexOf("U") >= 0) {
					...
					if (p >= 0)  N=0+parseInt(message.substr(1));
					p=message.indexOf("U");
					if (p >= 0)  N=0+parseInt(message.substr(1));
                    var step = N*RX_modes[SDR_RX.mode][2];
                    switch (ZoomFreq.id) {
                    case "DOF":
...
$("#CW_Zin").mousedown(function () { //Zero Beat
    if (SDR_RX.mode > 4 && audioRX.on && F_Audio_Top > 0) {
        $("#CW_Zin").removeClass('bt_off').addClass('bt_on');
        var deltaF = CW_para.pitch - F_Audio_Top; //CW-LSB
        if (SDR_RX.mode == 6)
