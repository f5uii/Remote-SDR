...
// ***********************************
const Version = "Remote SDR V4.4<br><a href='http://f1atb.fr' target='_blank'>F1ATB</a> January 2022";
const Version_Local_Storage = "4.4";
//Visus
...
function Trace_Audio() {
    var Oscillo = {
        H: $("#Oscillo").innerHeight(),
        W: $("#Oscillo").innerWidth()
    };
    ...
    if ((Tc - TraceAudio.T0) > 2000) {
        ...
        for (var i = 1; i < 20; i = i + 2) {
            var X = i * Oscillo.W / 20;
            ctx.moveTo(X, Oscillo.H);
            ctx.lineTo(X, TraceAudio.T);
            ctx.fillText(i / 10, X - Oscillo.W / 40, TraceAudio.T);
        }
        ctx.fillText("   s", X, TraceAudio.T);
        ctx.stroke();
        //Size MAX of the input Buffer adjusted to communication quality
        if (audioRX.BufferSize > 0.8 * audioRX.BufferMax) { //close to the max
            audioRX.BufferMax = audioRX.BufferMax + 500;
        } else {
            audioRX.BufferMax = audioRX.BufferMax - 50;
        }
        audioRX.BufferMax = Math.min(6000, Math.max(2000, audioRX.BufferMax)); //between 0.2s and 0.6s
        let S = (audioRX.BufferMax / 10000).toPrecision(2);
        $("#inOscillo").html("Input Audio Buffer Size (max:" + S + "s)");
    }
    TraceAudio.X = (Tc - TraceAudio.T0) * Oscillo.W / 2000;
    if (audioRX.Ctx != null) {
        ctx.beginPath();
        ctx.strokeStyle = "Aqua";
        if (Math.abs(audioRX.BufferSize - audioRX.BufferMax / 2) > audioRX.BufferMax / 2.2) {
            ctx.strokeStyle = "Red";
            audioRX.BufferMax = audioRX.BufferMax + 5;
        }
        ctx.moveTo(Xold, TraceAudio.Y);
        TraceAudio.Y = TraceAudio.H - audioRX.BufferSize * TraceAudio.H / audioRX.BufferMax;
...
function Trace_Spectre(spectre) {
    ...
    var Beam_Sector = 12; //Half sector spatial filter
    if (SDR_RX.mode == 4)
        Beam_Sector = 100; //WBFM
    Beam_Sector = Beam_Sector * bandes[SDR_RX.idx_bande][2]; //Decim factor
    Beam.Center = [];
    let arr1 = spectre.slice(Sl2, Sl); // Array is shifted by 1/2 in GNU Radio Block FFT Mag Log
    let arr2 = spectre.slice(0, Sl2);
    Beam.Center = [...arr1, ...arr2];
    for (var i = 0; i < Sl; i++) {
        Beam.Mean[i] = 0.1 * Beam.Center[i] + 0.9 * Beam.Mean[i]; //Mean values - First Order Filter
        var BS = Beam_Sector;
        if (i < Sm1)
        ...
        if (i > Sl2 && m < Sm3)
            m = Math.min(i - 2, Sm3);
        var sector = Beam.Mean.slice(m, n);
        Beam.Min[i] = Math.min(...sector);
        if (visus.spectre_lisse) {
            var Y = Math.floor(H * (1 - visus.spectre_haut * (Beam.Mean[i] - Beam.Min[i] + visus.spectre_bas)));
        } else {
            var Y = Math.floor(H * (1 - visus.spectre_haut * (Beam.Center[i] - Beam.Min[i] + visus.spectre_bas)));
        }
        S += X + ',' + Y + ' ';
        X += dX;
    }
    ...
    for (var i = idx_audio0; i <= idx_audio1; i++) {
        var j = Math.max(0, i);
        j = Math.min(Sl - 1, j);
        S_metre.level = Math.max(S_metre.level, Beam.Mean[j]);
        Amp_min = Math.min(Amp_min, Beam.Min[j]);
    }
    ...
    for (var idx = idx_audio_deb; idx <= idx_audio_fin; idx++) {
        if (idx >= 0 && idx < Sl) {
            var Y = H * visus.spectre_haut * (Beam.Mean[idx] - Beam.Min[i] + visus.spectre_bas) + 1;
...
function Estimate_Max_Freq(Idx0) {
    ...
    Idx0 = Math.max(5, Math.min(FFT - 5, Idx0))
        var Vg = Math.pow(10, Beam.Mean[Idx0 - 1] / 10000); //Linear and not dB
    var Vc = Math.pow(10, Beam.Mean[Idx0] / 10000);
    var Vd = Math.pow(10, Beam.Mean[Idx0 + 1] / 10000);
    var dIdx = 5 * (Vd - Vg) / Vc;
    dIdx = Math.max(-0.5, Math.min(0.5, dIdx))
        var Freq_of_Max = ((Idx0 + dIdx + 0.5) / FFT - 0.5) * SDR_RX.bande; //Freq fine of the maximum
    return Freq_of_Max;
}
function Trace_Waterfall() {
    ...
    var k = 0;
    for (var i = 0; i < Sl; i++) {
        var A = Math.floor(300 * Math.max(visus.water_haut * (Beam.Center[i] - Beam.Min[i] + visus.water_bas), 0));
        A = Math.min(A, 300);
...
function Trace_Echelle() { // Scale drawing
    ...
    for (var i = 0; i < Zone.length; i++) {
        if ((Zone[i][0] >= SDR_RX.min && Zone[i][0] <= SDR_RX.max) || (Zone[i][1] >= SDR_RX.min && Zone[i][1] <= SDR_RX.max) || (Zone[i][0] < SDR_RX.min && Zone[i][1] > SDR_RX.max)) {
            ctxE.beginPath();
...
function Affich_freq_Audio_RX() {
    ...
    SDR_RX.CW_shiftRXTX = 0
        ...
        SDR_RX.Audio_RX = Math.floor(SDR_RX.centrale_RX + SDR_RX.fine + SDR_RX.CW_shiftRXTX);
    $("#Fsaisie").html(FkHz(SDR_RX.Audio_RX));
    Affich_freq_champs(SDR_RX.Audio_RX, "#FRX");
    $("#CentFreq").html(FkHz(SDR_RX.centrale_RX) + " kHz");
    if (ZoomFreq.id == "FRX")
        Affich_freq_champs(SDR_RX.Audio_RX, "#ZFr"); //Zoom display
    Save_RX_Para();
    Affiche_Curseur();
}
...
function Affiche_Curseur() {
    var p = ecran.innerW * (0.5 + (SDR_RX.fine + SDR_RX.CW_shiftRXTX) / (SDR_RX.bande)) - 10 + ecran.border;
    $("#curseur").css("left", p);
}
...
function window_resize() {
    ...
    if (ecran.hauteur * 0.4 < 300) {
        $(".fenetres").css("height", "50%");
        $("#visus").css("top", "50%");
        $("#visus").css("height", "50%");
    }
...
function newBandRX(t) {
    SDR_RX.centrale_RX = Math.floor((BandesRX[t][0] + BandesRX[t][1]) / 2);
    SDR_RX.fine = Math.min(BandesRX[t][1] - SDR_RX.centrale_RX, SDR_RX.fine);
    SDR_RX.fine = Math.max(BandesRX[t][0] - SDR_RX.centrale_RX, SDR_RX.fine);
    choixBandeRX();
...
function ValideIP() {
    ...
    if (Host_IP.RXport != "")
        port = Host_IP.RXport;
	SDR_RX.port_audio = $("#RX_Port").val().trim();
	SDR_RX.port_audio=parseInt(SDR_RX.port_audio);
    if (SDR_RX.port_audio < 1024)
        SDR_RX.port_audio = "8001";
    $("#RX_ports").html("Ports: " + port + ", " + SDR_RX.port_audio + "," + (SDR_RX.port_audio + 1) + "," + (SDR_RX.port_audio + 2) + ", ( " + (SDR_RX.port_audio + 5) + "," + (SDR_RX.port_audio + 7) + " )");
    var port = "80";
    if (Host_IP.TXport != "")
        port = Host_IP.TXport;
    $("#TX_ports").html("Ports: " + port + ", " + (SDR_RX.port_audio + 3) + "," + (SDR_RX.port_audio + 4) + ", ( " + (SDR_RX.port_audio + 6) + " )");
...
function Host_ip() {
    var Host = SDR_RX.IP.split(":");
    Host_IP.RX = Host[0];
    if (Host.length > 1)
        Host_IP.RXport = Host[1];
    var Host = SDR_TX.IP.split(":");
    Host_IP.TX = Host[0];
    if (Host.length > 1)
        Host_IP.TXport = Host[1];
}
// Drawing
//**********
function Plot_Table(canvas_ID, tableau, SR, Fmax, couleur, Coef_Ampli, Trace_Axe, MaxF) { // dessine une onde (tableau de bytes) dans le canvas
...
function SetIframe(I) {
    if (Iframes.length > 0) {
        ...
        for (var i = 0; i < Iframes.length; i++) {
            var j = i + 1;
            s += '<div id ="ong' + j + '" class="onglet_If coral" onclick= "SetIframe(' + j + ');" >' + Iframes[i][1] + '</div>'
        }
        $("#menu_Iframe").html(s);
    ...
    if (I == 0) {
        ...
        var s = '<iframe style="width:100%;height:100%;" src="' + Iframes[I - 1][0] + '" frameBorder="0"></iframe>';
        $("#Iframe").html(s);
